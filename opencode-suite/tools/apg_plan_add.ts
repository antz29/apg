...
export default tool({
  ...
  args: {
    ...
    fqn: tool.schema
      .string()
      .optional()
      .describe("For task: the Implementation FQN the verb applies to (for renames/moves, the source FQN). On a task update, an explicitly empty string (\"\") clears the target to the target-less form (only with verb=creates), while omitting it leaves the target unchanged; the CLI refuses --fqn together with --no-fqn. For planned node: the real code FQN where the code will land (required)."),
  ...
  async execute(args, context) {
    ...
    if (action === "update") {
      if (kind === "phase") {
        ...
        if (args.fqn === "") cli.push("--no-fqn")
        else if (args.fqn) cli.push("--fqn", args.fqn)
