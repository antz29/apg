...
export default tool({
  ...
  async execute(args, context) {
    ...
    const sat = csvToRows(await runCypher(context, "MATCH (pp:PlanPhase)-[:Satisfies]->(r:Requirement) RETURN pp.fqn, r.fqn"))
    const tasks = csvToRows(await runCypher(context, "MATCH (pp:PlanPhase)-[:Contains]->(t:Task) RETURN pp.fqn, t.fqn, t.status"))
    const verbs = csvToRows(await runCypher(context, "MATCH (t:Task) RETURN t.fqn, t.verb, t.target"))
