...
export default tool({
  ...
  async execute(args, context) {
    ...
    const phases = csvToRows(
      await runCypher(
        context,
        `MATCH (p:Phase) WHERE p.fqn STARTS WITH ${lit(`${project}/spec.phase-`)} RETURN p.fqn, p.number, p.title ORDER BY p.number`,
      ),
    )
    ...
    for (const [from, to] of csvToRows(
      await runCypher(
        context,
        `MATCH (a:Phase)-[:Gates]->(b:Phase) WHERE a.fqn STARTS WITH ${lit(`${project}/spec.phase-`)} RETURN a.fqn, b.fqn`,
      ),
    )) {
      if (from === "a.fqn") continue
      const key = from.replace(`${project}/spec.phase-`, "")
      gates.set(key, [...(gates.get(key) ?? []), to.replace(`${project}/spec.phase-`, "")])
    }

    const lines = ["number,title,gated-on"]
    for (const [fqn, number, title] of phases.slice(1)) {
      const n = fqn.replace(`${project}/spec.phase-`, "")
