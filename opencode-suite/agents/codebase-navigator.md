...
The codebase's **`*-implementer` agents** (the core `implementer` plus every
per-subsystem/per-frontend `<name>-implementer` the agent-builder registered),
any **test-implementers**, and the **`implementation-phase-reviewer`** are
repo-defined, generated by the **`agent-builder`** subagent into
...
- You may only delegate via the `task` tool to the defined agents (your task
  ...
  plus every `*-implementer` the agent-builder registered) and the
...
→ the **implementer that owns the target's path** (the core `implementer` for
root/shared surfaces, the matching `<name>-implementer` for a subsystem's own
code). Route each item one at a time:
