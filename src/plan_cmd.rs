...
mod tests {
    ...
    mod unit {
        ...
        fn link_phase_edges_keeps_all_satisfies_and_preserves_other_phases() {
            ...
            assert!(
                link_phase_edges("foo/plan.phase-01", &[], &["2".into()], &mut records,).is_err()
            );
    ...
    mod e2e {
            ...
            let task = records
                ...
                .find_map(|r| match r {
                    Record::Task {
                        ...
                        Some((title.as_str(), status.as_str()))
                    }
                    _ => None,
                })
                .unwrap();
            ...
            let planned = records
                ...
                .find_map(|r| match r {
                    Record::PlannedNode {
                        ...
                        Some((kind.as_str(), name.as_str(), parent.as_str()))
                    }
                    _ => None,
                })
                .unwrap();
        ...
        fn task_verb_old_format_records_default_to_creates() {
            ...
            let dir =
                std::env::temp_dir().join(format!("apg-plan-verb-parse-{}", std::process::id()));
        ...
        fn plan_link_retired_and_update_phase_sets_bridge_edges() {
            ...
            let err =
                cmd_plan(&["link".to_string(), "foo".to_string(), "1".to_string()]).unwrap_err();
            assert!(
                err.to_string()
        ...
        fn plan_verify_gates_feedback_in_tier_mirrors() {
            ...
            let mirror = specs::transient_feedback_path(
                &apg_root,
                "foo",
                crate::layers::Layer::Requirements,
            );
        ...
        fn plan_rm_cascades_leave_no_orphan_records() {
            ...
            assert!(
                !recs.iter().any(
                    |r| matches!(r, Record::Contains { to, .. } if to == "github.com/x/y.Store")
                )
            );
            ...
            assert!(
                !recs.iter().any(
                    |r| matches!(r, Record::Feedback { fqn, .. } if fqn == "foo/feedback-task")
                ),
            ...
            assert!(
                !recs.iter().any(
                    |r| matches!(r, Record::PlanPhase { fqn, .. } if fqn == "foo/plan.phase-01")
                )
            );
            assert!(
                !recs
            ...
            assert!(
                !recs.iter().any(
                    |r| matches!(r, Record::Gates { from, .. } if from == "foo/plan.phase-02")
                ),
