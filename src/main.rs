...
mod tests {
    ...
    /// The per-phase timing report parsed from a scan's machine-readable
    /// `[timing-json] …` line. Parsing succeeds only when all four phases are
    /// present, so a suite of four is asserted structurally.
    fn scan_timing(stderr: &str) -> crate::timing::TimingReport {
        let line = stderr
            .lines()
            .rev()
    ...
    /// One phase's measured milliseconds.
    fn phase_ms(report: &crate::timing::TimingReport, phase: crate::timing::Phase) -> f64 {
        report.duration(phase).as_secs_f64() * 1000.0
    }

    /// Asserts the human line carries all four phase keys (zero where no work
    /// ran) and the machine line round-trips all four durations.
    fn assert_all_four_phases(stderr: &str) -> crate::timing::TimingReport {
        assert!(
            stderr.contains("[timing]"),
            "the human timing line: {stderr}"
        );
    ...
    /// The four phases as a JSON object of milliseconds (for the artifacts).
    fn phases_json(report: &crate::timing::TimingReport) -> serde_json::Value {
        use crate::timing::Phase;
        serde_json::json!({
            "startup_ms": phase_ms(report, Phase::Startup),
            "frontend_ms": phase_ms(report, Phase::Frontend),
            "ingest_assembly_ms": phase_ms(report, Phase::IngestAssembly),
            "db_load_ms": phase_ms(report, Phase::DbLoad),
        })
    }

    /// `[scan] incremental: N target file(s), …` → N.
    fn incremental_target_count(stderr: &str) -> Option<usize> {
        let prefix = "[scan] incremental: ";
        let at = stderr.find(prefix)? + prefix.len();
        let rest = &stderr[at..];
        rest[..rest.find(" target")?].trim().parse().ok()
    }

    /// `signature change cascades to N dependent file(s)` → N.
    fn cascade_count(stderr: &str) -> Option<usize> {
        let prefix = "signature change cascades to ";
        let at = stderr.find(prefix)? + prefix.len();
        let rest = &stderr[at..];
        rest[..rest.find(" dependent")?].trim().parse().ok()
    }

    /// The first stderr line containing `needle` (for the recorded artifacts).
    ...
    fn exported_java_files(repo: &Path, limit: usize) -> Vec<PathBuf> {
        ...
        for line in text.lines() {
            ...
            if v.get("type").and_then(|t| t.as_str()) == Some("file")
                && v.get("code_type").and_then(|c| c.as_str()) == Some("src")
            {
    ...
    /// preserving byte length and line count so no declaration signature or span
    /// changes (the localized set stays localized — no cascade).
    fn body_only_edit(path: &Path) {
            ...
            if trimmed.starts_with("//") || trimmed.starts_with('*') {
                for (j, b) in line.bytes().enumerate() {
                    if b.is_ascii_alphabetic() {
                        bytes[offset + j] = if b == b'a' { b'b' } else { b'a' };
        ...
        panic!("no comment line to edit in {}", path.display());
    }

    /// The most-`uses`-depended-on `src` Struct of a scan export, as
    /// `(fqn, absolute source path)` — the task-9 signature-change target.
    fn top_depended_on_struct(repo: &Path) -> (String, PathBuf) {
        let text = std::fs::read_to_string(repo.join("apg/.trans/graph.jsonl")).unwrap();
        let mut uses: std::collections::BTreeMap<String, usize> = std::collections::BTreeMap::new();
        let mut paths: std::collections::BTreeMap<String, PathBuf> =
            ...
            match v.get("type").and_then(|t| t.as_str()) {
                Some("uses") => {
                    if let Some(to) = v.get("to").and_then(|x| x.as_str()) {
                        *uses.entry(to.to_string()).or_default() += 1;
                    }
                }
                Some("struct") if v.get("code_type").and_then(|c| c.as_str()) == Some("src") => {
                    if let (Some(fqn), Some(path)) = (
                        v.get("fqn").and_then(|x| x.as_str()),
                        v.get("path").and_then(|x| x.as_str()),
                    ) {
                        paths.insert(fqn.to_string(), PathBuf::from(path));
                    }
                }
                _ => {}
            }
        ...
        paths
            .keys()
            .filter_map(|fqn| uses.get(fqn).map(|n| (*n, fqn.clone())))
            .max_by_key(|(n, _)| *n)
            .map(|(_, fqn)| (fqn.clone(), paths[&fqn].clone()))
            .expect("the export must contain a depended-on src Struct")
    }

    /// Phase-04 acceptance setup: `apg init` + commit, then the cold FULL scan
    /// that records the jgrapht baseline. Returns the harness, the baseline
    /// wall-clock ms, the baseline report, and the cold stderr.
    fn acceptance_baseline(
        h: testutil::AcceptanceHarness,
    ) -> (
        testutil::AcceptanceHarness,
        f64,
        ...
        testutil::commit_all_files(&h.repo, "apg init");

        let t = std::time::Instant::now();
        let cold = h.run(&["scan", "."]);
        let wall_ms = t.elapsed().as_secs_f64() * 1000.0;
        let err = String::from_utf8_lossy(&cold.stderr).into_owned();
        assert!(cold.status.success(), "cold baseline scan: {err}");
        assert!(
            err.contains("Languages: java"),
            "the isolated Java frontend must be the only language: {err}"
        );
        let report = assert_all_four_phases(&err);
        assert!(
            !err.contains("frontend-skipped"),
            "the cold baseline is a full scan: {err}"
        );
        (h, wall_ms, report, err)
    }
    ...
    mod e2e {
        ...
        // -------------------------------------------------------------------
        // Phase-04 acceptance scenarios (tasks 7–9): the jgrapht reference
        // workload, staged read-only into a scratch /tmp copy and driven by the
        // CANDIDATE binary only (global.constraint.no-real-project-test).
        // Opt-in e2e: each is #[ignore]d and run via `cargo test-e2e`.
        // -------------------------------------------------------------------

        /// Phase-04 task-7 — fresh-worktree near-instant. A cold full scan of
        /// the staged jgrapht copy records the baseline; `apg project start`
        /// creates a fresh worktree (auto-scanning it), and the measured RE-SCAN
        /// of that already-scanned, unchanged worktree must take the Win-A
        /// whole-tree fast path: all four phase durations on both the human and
        /// machine lines, the frontend phase marked `frontend-skipped`, no
        /// frontend spawn, no DB rebuild, and a total under the stated
        /// near-instant target (≤2s and ≤15% of the recorded baseline).
        #[test]
        #[ignore = "e2e tier: real I/O (staged jgrapht checkout/spawned apg/db.lbug); run via cargo test-e2e"]
        fn acceptance_fresh_worktree_near_instant_jgrapht() {
            let h = match testutil::AcceptanceHarness::jgrapht() {
                Ok(h) => h,
                Err(e) if e.is_skip() => {
                    eprintln!("{e}");
                    return;
            ...
            // `apg project start` creates the worktree AND auto-scans it.
            let start = h.run(&["project", "start", "fresh"]);
            assert!(
            ...
            let wt = h.repo.join("apg/.worktrees/fresh");
            let db = wt.join("apg/.trans/db.lbug");
            assert!(
                db.is_file(),
                "project start must leave the worktree with its own db.lbug"
            );
            let db_before = std::fs::read(&db).unwrap();

            // The MEASURED RUN: a re-scan of the unchanged, already-scanned
            // worktree — the whole-tree freshness fast path.
            let t = std::time::Instant::now();
            let rescan = h.run_in(&wt, &["scan", "."]);
            let rescan_ms = t.elapsed().as_secs_f64() * 1000.0;
            let err = String::from_utf8_lossy(&rescan.stderr).into_owned();
            assert!(rescan.status.success(), "worktree re-scan: {err}");
            let report = assert_all_four_phases(&err);
            assert!(
                err.contains("frontend-skipped"),
                "the fast path must mark the frontend phase skipped on the human line: {err}"
            );
            assert!(
                report.frontend_skipped(),
                "the machine line must carry frontend_skipped"
            );
            assert!(
                !err.contains("[scan] running"),
                "the fast path must spawn no frontend: {err}"
            );
            assert_eq!(
                std::fs::read(&db).unwrap(),
                db_before,
            ...
            let target_2s = 2000.0_f64;
            let target_pct = baseline_ms * 0.15;
            let artifact = serde_json::json!({
                ...
                "baseline_phases_ms": phases_json(&base_phases),
                "rescan_ms": rescan_ms,
                "rescan_phases_ms": phases_json(&report),
                "stated_target_ms": { "abs_2s": target_2s, "pct_15_of_baseline": target_pct },
                "frontend_skipped": report.frontend_skipped(),
                "pass": rescan_ms <= target_2s && rescan_ms <= target_pct,
            });
            let path =
                testutil::write_acceptance_artifact("fresh-worktree-jgrapht.json", &artifact);
            assert!(
                rescan_ms <= target_2s,
                "near-instant target: the re-scan was {rescan_ms:.1}ms, must be ≤ {target_2s:.0}ms (artifact {})",
                path.display()
            );
            assert!(
                rescan_ms <= target_pct,
                "near-instant target: the re-scan was {rescan_ms:.1}ms, must be ≤ 15% of the {baseline_ms:.1}ms baseline ({target_pct:.1}ms) (artifact {})",
                path.display()
            );
        ...
        fn acceptance_localized_edit_jgrapht_incremental() {
            ...
            assert!(
                !files.is_empty(),
                "the jgrapht export must contain src .java files"
            );
            for f in &files {
                body_only_edit(f);
            }
            ...
            let err = String::from_utf8_lossy(&inc.stderr).into_owned();
            assert!(inc.status.success(), "localized re-scan: {err}");
            let report = assert_all_four_phases(&err);
            assert!(
                err.contains("[scan] incremental:"),
                "the incremental verdict must be printed: {err}"
            );
            assert!(
                !err.contains("frontend-skipped"),
                "a content edit clears the whole-tree fast-path marker: {err}"
            );
            assert!(
                cascade_count(&err).is_none(),
                "a body-only localized edit must not cascade: {err}"
            );
            let targets = incremental_target_count(&err).unwrap_or(0);
            assert!(
                (1..=5).contains(&targets),
                "the localized target set must be 1–5 files (got {targets}): {err}"
            );
            assert!(
                err.contains("[load] splice:"),
                "the P3 splice path must be taken: {err}"
            );
            assert!(
                !err.contains("[load] writing parquet load files"),
        ...
        /// Phase-04 task-9 — a signature change to a widely-depended-on jgrapht
        /// symbol. The run may approach the full-scan baseline (near-full is NOT
        /// a failure; the duration is recorded), but it must report all four
        /// phases, re-emit the reverse-dependency closure, and be EXACT: the
        /// incremental graph equals a from-scratch FULL scan of the SAME changed
        /// tree (DB + shared fact cache cleared) under the enumerated oracle —
        /// per-label counts, per-rel-type counts, the UnresolvedTarget set with
        /// categories, the Scan row, and `graph.jsonl`.
        #[test]
        #[ignore = "e2e tier: real I/O (staged jgrapht checkout/spawned apg/db.lbug); run via cargo test-e2e"]
        fn acceptance_wide_signature_change_jgrapht_exact() {
            let h = match testutil::AcceptanceHarness::jgrapht() {
                Ok(h) => h,
                Err(e) if e.is_skip() => {
                    eprintln!("{e}");
            ...
            let (h, baseline_ms, _, _) = acceptance_baseline(h);

            // The widely-depended-on symbol (most incoming `uses` edges) and its
            // source file, discovered from the cold export.
            let (fqn, path) = top_depended_on_struct(&h.repo);
            let text = std::fs::read_to_string(&path).unwrap();
            let simple = fqn.rsplit('.').next().unwrap_or(&fqn).to_string();
            assert!(
                text.contains(&simple),
                "the discovered symbol {fqn} must live in {}",
                path.display()
            );
            let is_interface = text.contains(&format!("interface {simple}"))
                && !text.contains(&format!("@interface {simple}"));
            let kw = if is_interface { "default" } else { "public" };
            let marker = format!("\n\n    {kw} void apgAcceptanceProbe() {{\n    }}\n");
            let idx = text.rfind('}').expect("a closing brace in the type file");
            let edited = format!("{}{}{}", &text[..idx], marker, &text[idx..]);
            ...
            let inc_ms = t.elapsed().as_secs_f64() * 1000.0;
            let err = String::from_utf8_lossy(&inc.stderr).into_owned();
            assert!(inc.status.success(), "wide-signature re-scan: {err}");
            let report = assert_all_four_phases(&err);
            assert!(
                err.contains("[scan] incremental:"),
                "the incremental verdict must be printed: {err}"
            ...
            assert!(
                ...
                "the reverse-dependency closure must be re-emitted: {err}"
            );
            let cascaded = cascade_count(&err).unwrap_or(0);
            assert!(cascaded > 0, "the cascade must be non-empty: {err}");

            // Snapshot the incremental pair, then a fresh FULL scan of the SAME
            // changed tree with the fast-path, the DB AND the shared fact cache
            // cleared — never the incremental run compared against itself.
            let trans = h.repo.join("apg/.trans");
            let inc_db = h.base.join("inc.lbug");
            let inc_jsonl = h.base.join("inc.jsonl");
            std::fs::copy(trans.join("db.lbug"), &inc_db).unwrap();
            std::fs::copy(trans.join("graph.jsonl"), &inc_jsonl).unwrap();
            std::fs::remove_file(trans.join("db.lbug")).unwrap();
            std::fs::remove_file(trans.join("graph.jsonl")).unwrap();
            let _ = std::fs::remove_dir_all(h.repo.join(".git/apg/facts"));
            let full = h.run(&["scan", "."]);
            let full_err = String::from_utf8_lossy(&full.stderr).into_owned();
            assert!(full.status.success(), "fresh full-scan oracle: {full_err}");
            let full_db = trans.join("db.lbug");
            let full_jsonl = trans.join("graph.jsonl");

            // The enumerated exactness oracle. Every divergence is collected
            // (and written to the artifact) BEFORE the assertion, so a failing
            // acceptance records exactly what differs.
            let inc_counts = testutil::db_table_counts(&inc_db);
            let full_counts = testutil::db_table_counts(&full_db);
            let inc_unres = testutil::db_unresolved_rows(&inc_db);
            let full_unres = testutil::db_unresolved_rows(&full_db);
            let inc_scan = testutil::db_scan_row(&inc_db);
            let full_scan = testutil::db_scan_row(&full_db);
            let inc_text = std::fs::read_to_string(&inc_jsonl).unwrap();
            let full_text = std::fs::read_to_string(&full_jsonl).unwrap();
            let canon = |t: &str| -> BTreeSet<String> {
                t.lines()
            ...
            for label in ["Module", "File", "Struct", "Function", "UnresolvedTarget"] {
                assert!(
                    inc_counts.contains_key(label),
                    "the oracle must enumerate the {label} node table: {inc_counts:?}"
                );
            }
            for table in [
                "Contains",
                ...
                assert!(
                    inc_counts.contains_key(table),
                    "the oracle must enumerate the {table} rel table: {inc_counts:?}"
                );
            }
            for (table, a) in &inc_counts {
                match full_counts.get(table) {
                    Some(b) if a == b => {}
                    Some(b) => mismatches.push(format!("{table} count {a} != full {b}")),
                    None => mismatches.push(format!("{table} absent from the full scan")),
                }
            }
            if inc_unres != full_unres {
                mismatches.push(format!(
                    "UnresolvedTarget set differs: missing {} (e.g. {:?}); extra {} (e.g. {:?})",
                    full_unres.difference(&inc_unres).count(),
                    full_unres
                        .difference(&inc_unres)
                        .take(5)
                        .collect::<Vec<_>>(),
                    inc_unres.difference(&full_unres).count(),
                    inc_unres
                        .difference(&full_unres)
                        .take(5)
                        .collect::<Vec<_>>(),
                ));
            }
            if (inc_scan.0.clone(), inc_scan.1.clone())
                != (full_scan.0.clone(), full_scan.1.clone())
            {
                mismatches.push("the Scan row's sha/clean differs".to_string());
            }
            let inc_canon = canon(&inc_text);
            let full_canon = canon(&full_text);
            if inc_canon != full_canon {
                mismatches.push(format!(
                    "graph.jsonl non-scan_meta records differ: missing {} (e.g. {:?}); extra {} (e.g. {:?})",
                    full_canon.difference(&inc_canon).count(),
                    full_canon.difference(&inc_canon).take(3).collect::<Vec<_>>(),
                    inc_canon.difference(&full_canon).count(),
                    inc_canon.difference(&full_canon).take(3).collect::<Vec<_>>(),
                ));
            }
            // The export must round-trip through the re-ingest reader.
            let back = crate::load::tests::read_graph_jsonl(&inc_jsonl).unwrap();
            let again = h.base.join("again.jsonl");
            crate::load::write_graph_jsonl(&back, &again).unwrap();
            let again_text = std::fs::read_to_string(&again).unwrap();
            if canon(&inc_text) != canon(&again_text)
                || again_text.lines().next().unwrap() != inc_text.lines().next().unwrap()
            {
                mismatches
                    .push("graph.jsonl did not round-trip through read_graph_jsonl".to_string());
            }
            ...
            let artifact = serde_json::json!({
                ...
                "staged_files": h.staged_files,
                "frontends": "java-only (isolated APG_FRONTEND_DIR)",
                "changed_symbol_fqn": fqn,
                "changed_source_path": path.display().to_string(),
                "inserted_member": format!("{kw} void apgAcceptanceProbe()"),
                "baseline_full_scan_ms": baseline_ms,
                "incremental_ms": inc_ms,
                "incremental_phases_ms": phases_json(&report),
                "cascade_dependent_files": cascaded,
                "incremental_verdict": line_with(&err, "[scan] incremental:"),
                "cascade_verdict": line_with(&err, "signature change cascades"),
                "near_full": inc_ms >= baseline_ms * 0.5,
                "exact_vs_fresh_full_scan": exact,
                "mismatches": mismatches.clone(),
                "pass": exact,
            });
            let artifact_path =
                testutil::write_acceptance_artifact("wide-signature-jgrapht.json", &artifact);
            assert!(
                exact,
                "the incremental graph must equal a fresh full scan of the same tree: {} (artifact {})",
                mismatches.join("; "),
