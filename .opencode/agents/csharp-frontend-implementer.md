...
`src/csharplib/` — is the C# scanner: a standalone single-file .NET program
built on Roslyn (`Microsoft.CodeAnalysis.CSharp`) that resolves calls/types
exactly and streams one JSON object per line (declarations, references, edges)
to stdout for the Rust ingestor. You own your frontend's source; you never touch
another frontend, the root crate, or `.opencode/**`.
...
- **Your crate**: `src/csharplib/` — `Program.cs` (the scanner) and
  `CsharpFrontend.csproj` (the project file). It is a standalone single-file
  .NET program built on Roslyn (`Microsoft.CodeAnalysis.CSharp`) for exact
  call/type resolution; `build.rs` compiles it with `dotnet` and stages the
...
- **No pattern contains `&&`, `|`, `;`, `$()`/`$(...)`, or redirection — a
  ...
  call. `dotnet build && dotnet run` is denied; run them as separate calls.
